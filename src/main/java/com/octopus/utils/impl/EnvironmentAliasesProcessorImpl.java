...
public class EnvironmentAliasesProcessorImpl implements EnvironmentAliasesProcessor {
    @Override
    public void addEnvironmentVarsAsAliases() {
        /*
    ...
    @Override
    public void addSystemPropVarsAsAliases() {
        AutomatedBrowserBase.setExternalAliases(
                System.getProperties().entrySet().stream()
                        .filter(s -> s.getKey().toString().startsWith(Constants.ALIAS_HEADER_PREFIX))
                        .collect(Collectors.toMap(x ->
                                        x.getKey().toString().replaceAll(Constants.ALIAS_HEADER_PREFIX, ""),
